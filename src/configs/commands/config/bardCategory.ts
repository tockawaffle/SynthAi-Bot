...
export default async function (interaction: CommandInteraction) {
    ...
    const bardCategory = interaction.options.get("category", true)
        .value as string;
    ...
    if (checkCategory.type !== 4) {
        ...
        const catPerms = await setCategoryPermissions(interaction, bardCategory);
        if(!catPerms) {
            return await interaction.reply({
        ...
        if (server) {
            await serverSchema.findOneAndUpdate(
                ...
                {
                    $set: {
                        "channels.bardCategory": bardCategory,
                    },
                },
                { upsert: true }
            );
            ...
            return await interaction.reply({
                content: translate(user, "config", "bardCategorySet").replace(
                    "%s",
                    `${checkCategory}`
                ),
            });
